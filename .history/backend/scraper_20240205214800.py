...
import contractions
import regex as re
from config import API_URL, KEYWORDS, MAX_PAGES
...
class JobScrapper(object):
    ...
    def scrape_article_ids(self, api_url, max_pages):
        ...
        return self.job_id, self.titles, self.companies, self.locations, self.categorys, self.subCategorys, self.job_types, self.salarys

    def fetch_job_article(self):
        article_url = f'https://www.jobstreet.com.my/job/{self.job_id}'
    ...
    def scrape_and_store_text(self, job_ids):
    ...
    def get_job_info(self, API_URL, MAX_PAGES):
        ...
        job_id, titles, companies, locations, categorys, subCategorys,job_types,salarys = self.scrape_article_ids(API_URL, MAX_PAGES)

        data = self.scrape_and_store_text(job_id)
