...
#String processing 
def clean_text(str_obj):
  cleaned_text = [s.replace('\n', '')
                    .replace('"','')
                    .replace(",",'')
                    .replace("\u2022",'') #u2022 is unicode for bullet points
                    for s in str_obj]
  #Remove empty elements within the list
...
def find_persons(obj):
    ...
    persons = [ent.text for ent in obj.ents if ent.label_ == 'PERSON']
    # Return persons
    return persons
...
def find_orgs(obj):
    ...
    org = [ent.text for ent in obj.ents if ent.label_ == 'ORG']
    # Return persons
    return org
